...
DELIMITER $$
CREATE TRIGGER update_points_scored_trigger
	AFTER INSERT
...
BEGIN
    ...
    UPDATE virtual_team SET totalPoints = totalPoints + pointsScored
    WHERE id IN ( SELECT DISTINCT virtual_team_id FROM virtual_team_player WHERE player_id = NEW.player_id );
END$$
DELIMITER ;